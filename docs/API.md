...
`GET /api/topology` returns a live in-memory view of remote IPv4 traffic addressed to configured service destination ports. Sources are grouped automatically by `BAZALT_TOPOLOGY_GROUP_PREFIX_V4` (default `/24`). The response contains per-group and per-source cumulative bytes/packets plus 5-second PPS/bit-rate estimates and a top-level `service_ports` array used by the star-map center. Off-service traffic is intentionally absent from topology/statistics. Per-source state is bounded by `BAZALT_TOPOLOGY_MAX_SOURCES` (default `65536`); overflow of service-directed traffic remains included in aggregate totals/rates and is exposed via the `untracked_*` fields. To keep the control plane responsive during high-cardinality floods, a snapshot returns at most the 256 busiest groups and 2048 busiest source rows. `group_count`, `source_count`, aggregate totals/rates, `returned_*` fields and `view_truncated` make this explicit rather than silently losing accounting.

The response also includes `enforcement`: whether XDP throttle is available, `BAZALT_INTERFACE`, the effective XDP attach mode (`native-link`, `native-netlink`, or `generic-skb-netlink`), active rules and recent in-memory audit entries. Topology filtering does not constrain enforcement: after a rule is installed, its XDP LPM lookup is keyed only by source IPv4/prefix, so the selected drop percentage applies to all destination ports from that source.
...
`target` may be one IPv4 address (`/32`) or exactly one automatically discovered group CIDR such as `10.10.1.0/24`. Intermediate or broader prefixes are rejected, so the default `/24` layout cannot accidentally install `/25`, `/16`, `/8` or `/0` penalties. `drop_percent` is `1..100`. `ttl_seconds` must be `1..3600`; omitted TTL defaults to 300 seconds. Permanent packet-drop rules are deliberately unsupported so a userspace crash cannot leave an indefinite punishment behind.
...
The throttle endpoint returns `503` when `BAZALT_THROTTLE_ENABLED=false`. Enforcement always attaches to `BAZALT_INTERFACE`; no independent throttle interface is configurable. If AF_XDP was requested while enforcement is enabled, BAZALT uses the throttle XDP program for ingress enforcement and automatically selects passive libpcap as the effective capture backend.
