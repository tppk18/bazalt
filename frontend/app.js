...
const state = {
  ...
  management: null, managementOpen: false, topology: null, topologyOpen: false, topologyLoading: false, topologyRuleSamples: new Map(),
};
...
function renderResources(snapshot,now=Date.now()){
  ...
  $('#resource-ingress-meta').textContent=captureByteRate===null?`${fmtCount(m.capture_frames)} backend frames total · diagnostic/pre-service`:`${(captureByteRate*8/1e6).toFixed(captureByteRate*8/1e6>=10?1:2)} Mbit/s · ${fmtBytes(captureByteRate)}/s · diagnostic/pre-service`;
...
function topologyRuleRates(rules){
  const now=performance.now();const live=new Set();const enriched=(rules||[]).map(rule=>{
    const target=String(rule.target||'');live.add(target);const cur={seenPackets:Number(rule.seen_packets||0),seenBytes:Number(rule.seen_bytes||0),droppedPackets:Number(rule.dropped_packets||0),droppedBytes:Number(rule.dropped_bytes||0),at:now};const prev=state.topologyRuleSamples.get(target);let seenPps=0,seenBps=0,droppedPps=0,droppedBps=0;
    if(prev){const dt=(now-prev.at)/1000;if(dt>0&&cur.seenPackets>=prev.seenPackets&&cur.seenBytes>=prev.seenBytes&&cur.droppedPackets>=prev.droppedPackets&&cur.droppedBytes>=prev.droppedBytes){seenPps=(cur.seenPackets-prev.seenPackets)/dt;seenBps=(cur.seenBytes-prev.seenBytes)*8/dt;droppedPps=(cur.droppedPackets-prev.droppedPackets)/dt;droppedBps=(cur.droppedBytes-prev.droppedBytes)*8/dt;}}
    state.topologyRuleSamples.set(target,cur);return {...rule,matched_packets_per_second:seenPps,matched_bits_per_second:seenBps,dropped_packets_per_second:droppedPps,dropped_bits_per_second:droppedBps};
  });
  for(const target of state.topologyRuleSamples.keys())if(!live.has(target))state.topologyRuleSamples.delete(target);return enriched;
}
function renderTopologyAudit(entries){const body=$('#topology-audit-body');if(!body)return;const rows=entries||[];body.innerHTML=rows.length?rows.map(entry=>`<tr><td>${esc(fmtTime(entry.at))}</td><td>${esc(String(entry.action||'').toUpperCase())}</td><td>${esc(entry.target||'-')}</td><td>${entry.drop_percent?`${esc(entry.drop_percent)}%`:'-'}</td><td>${entry.action==='set'?(entry.ttl_seconds?esc(fmtDuration(entry.ttl_seconds)):'UNKNOWN'):'-'}</td></tr>`).join(''):'<tr><td colspan="5">No throttle actions yet.</td></tr>';}
function renderTopologyRules(rules){const body=$('#topology-rules-body');if(!body)return;const rows=rules||[];body.innerHTML=rows.length?rows.map(rule=>`<tr><td>${esc(rule.target||'-')}</td><td><strong>${esc(rule.drop_percent)}%</strong></td><td>${rule.expires_at?esc(fmtTime(rule.expires_at)):'UNKNOWN'}</td><td><strong>${fmtBitRate(rule.matched_bits_per_second||0)}</strong><br><small>${fmtCount(rule.matched_packets_per_second||0)} pkt/s · ${fmtCount(rule.seen_packets||0)} total</small></td><td><strong>${fmtBitRate(rule.dropped_bits_per_second||0)}</strong><br><small>${fmtCount(rule.dropped_packets_per_second||0)} pkt/s · ${fmtCount(rule.dropped_packets||0)} total</small></td><td><button class="btn btn-sm btn-outline-warning" data-clear-throttle="${esc(rule.target||'')}">DISABLE</button></td></tr>`).join(''):'<tr><td colspan="6">No active throttle rules.</td></tr>';$$('[data-clear-throttle]').forEach(button=>button.addEventListener('click',()=>disableThrottleTarget(button.dataset.clearThrottle)));}
function ruleMeta(rule,sourceTarget=''){if(!rule)return '';const inherited=sourceTarget&&rule.target!==sourceTarget?` via ${rule.target}`:'';const expiry=rule.expires_at?` · until ${fmtTime(rule.expires_at)}`:' · expiry unknown';const dropped=Number(rule.dropped_packets||0)?` · ${fmtCount(rule.dropped_packets)} dropped`:'';return `${inherited}${expiry}${dropped}`;}
function renderTopologyStar(groups,t,rules,servicePorts){
  const root=$('#topology-star');if(!root)return;
  const ports=(servicePorts||[]).length?(servicePorts||[]).map(port=>`:${esc(port)}`).join(' · '):'NO SERVICE PORTS';
  if(!groups.length){root.innerHTML=`<div class="topology-star-center"><span>BAZALT</span><strong>${ports}</strong><small>waiting for service ingress</small></div><div class="topology-star-hidden topology-star-wait">Waiting for traffic to configured service ports…</div>`;return;}
  const shown=groups.slice(0,12);const cx=500,cy=310,rx=345,ry=230;const totalBps=Math.max(1,Number(t.bits_per_second)||0);
  const lineSvg=shown.map((group,i)=>{const angle=(-Math.PI/2)+(Math.PI*2*i/shown.length);const x=cx+Math.cos(angle)*rx;const y=cy+Math.sin(angle)*ry;const share=Math.max(0,Number(group.bits_per_second||0)/totalBps);const width=(1.2+Math.min(7,share*12)).toFixed(2);return `<line x1="${cx}" y1="${cy}" x2="${x.toFixed(1)}" y2="${y.toFixed(1)}" stroke-width="${width}"/>`;}).join('');
  const nodes=shown.map((group,i)=>{const angle=(-Math.PI/2)+(Math.PI*2*i/shown.length);const x=cx+Math.cos(angle)*rx;const y=cy+Math.sin(angle)*ry;const groupRule=rules.find(rule=>rule.target===group.cidr);const topSources=(group.sources||[]).slice(0,3);const sourceRows=topSources.map(src=>{const rule=effectiveTopologyRule(src.ip,rules);return `<button type="button" class="topology-star-source ${rule?'throttled':''}" data-throttle-target="${esc(src.ip)}" title="Throttle ${esc(src.ip)}"><span>${esc(src.ip)}</span><small>${fmtBitRate(src.bits_per_second||0)}</small></button>`;}).join('');const more=Math.max(0,Number(group.source_count||0)-topSources.length);return `<article class="topology-star-node ${groupRule?'throttled':''}" style="left:${(x/10).toFixed(2)}%;top:${(y/6.2).toFixed(2)}%"><button type="button" class="topology-star-group" data-throttle-target="${esc(group.cidr)}" title="Throttle ${esc(group.cidr)}"><strong>${esc(group.cidr)}</strong><span>${fmtBitRate(group.bits_per_second||0)} · ${fmtCount(group.packets_per_second||0)} pkt/s</span>${groupRule?`<em>DROP ${groupRule.drop_percent}%</em>`:''}</button><div class="topology-star-sources">${sourceRows}${more?`<small class="topology-star-more">+${fmtCount(more)} source${more===1?'':'s'}</small>`:''}</div></article>`;}).join('');
  const hidden=Math.max(0,groups.length-shown.length);
  root.innerHTML=`<svg class="topology-star-lines" viewBox="0 0 1000 620" preserveAspectRatio="none" aria-hidden="true">${lineSvg}</svg><div class="topology-star-center"><span>BAZALT</span><strong>${ports}</strong><small>${fmtBitRate(t.bits_per_second||0)} · ${fmtCount(t.packets_per_second||0)} pkt/s</small></div>${nodes}${hidden?`<div class="topology-star-hidden">+${fmtCount(hidden)} groups in details</div>`:''}`;
}
function renderTopology(data){
  const t=data?.traffic||{};const e=data?.enforcement||{};const groups=t.groups||[];const servicePorts=data?.service_ports||[];const rules=topologyRuleRates(e.rules||[]);if(data?.enforcement)data.enforcement.rules=rules;state.topology=data;
  ...
  $('#topology-source-count').textContent=fmtCount(t.source_count||0);$('#topology-source-ttl').textContent=`service ports ${servicePorts.length?servicePorts.map(p=>`:${p}`).join(', '):'none'} · hide after ${fmtDuration(t.source_ttl_seconds||0)} idle`;
  const capacityNote=$('#topology-capacity-note');const saturated=Boolean(t.source_capacity_saturated);capacityNote.classList.toggle('hidden',!saturated);capacityNote.textContent=saturated?`Source tracking cap reached (${fmtCount(t.source_capacity||0)}). Overflow traffic to configured service ports is still counted in aggregate totals but cannot be grouped per source: ${fmtBitRate(t.untracked_bits_per_second||0)}, ${fmtCount(t.untracked_packets_per_second||0)} pkt/s.`:'';
  const viewNote=$('#topology-view-note');const viewTruncated=Boolean(t.view_truncated);viewNote.classList.toggle('hidden',!viewTruncated);viewNote.textContent=viewTruncated?`Large topology: showing the busiest ${fmtCount(t.returned_group_count||0)} of ${fmtCount(t.group_count||0)} groups and ${fmtCount(t.returned_source_count||0)} of ${fmtCount(t.source_count||0)} tracked sources. Aggregate rates and totals still include all service-directed traffic.`:'';
  $('#topology-ingress-rate').textContent=fmtBitRate(t.bits_per_second||0);$('#topology-ingress-pps').textContent=`${fmtCount(t.packets_per_second||0)} packets/s to configured ports · ${t.rate_window_seconds||5}s window`;
  $('#topology-enforcement').textContent=e.available?'READY':'OFF';$('#topology-enforcement-meta').textContent=e.available?`${e.interface||'-'} · ${e.attach_mode||'xdp'} · ${rules.length} active rule${rules.length===1?'':'s'}`:'set BAZALT_THROTTLE_ENABLED=true';
  const note=$('#topology-enforcement-note');note.className=`topology-enforcement-note ${e.available?'ok':'warn'}`;note.textContent=e.available?`Topology/statistics include only remote IPv4 packets addressed to configured service ports. Once you throttle a visible source or /${t.group_prefix_v4??24} group, XDP enforcement is source-wide on ${e.interface}: the selected percentage is dropped across all destination ports. Active Rules shows pre-drop matched/drop rates from kernel counters.`:'Topology is service-port scoped. Enforcement is disabled until BAZALT_THROTTLE_ENABLED=true; when enabled, source-wide drop always uses BAZALT_INTERFACE.';
  renderTopologyRules(rules);renderTopologyAudit(e.audit||[]);renderTopologyStar(groups,t,rules,servicePorts);
  if(!groups.length){$('#topology-groups').innerHTML='<div class="topology-empty">Waiting for traffic to configured service ports…</div>';return;}
  ...
  $('#topology-groups').innerHTML=groups.map(group=>{
    ...
    return `<section class="topology-group"><div class="topology-group-head"><div class="topology-group-title"><strong>${esc(group.cidr)}</strong><span>${sourceLabel} · ${groupShare.toFixed(1)}% of service ingress</span></div><div class="topology-group-rate"><strong>${fmtBitRate(group.bits_per_second||0)}</strong><small>${fmtCount(group.packets_per_second||0)} pkt/s · ${fmtBytes(group.bytes_total||0)} service-directed total</small>${groupRule?`<span class="topology-rule-meta">DROP ${groupRule.drop_percent}%${esc(ruleMeta(groupRule))}</span>`:''}</div><button class="btn btn-sm ${groupRule?'btn-outline-warning':'btn-secondary'}" data-throttle-target="${esc(group.cidr)}">GROUP THROTTLE</button></div><div class="topology-share"><span style="width:${groupShare}%"></span></div><table class="topology-source-table"><thead><tr><th>SOURCE</th><th>LIVE RATE</th><th>GROUP SHARE</th><th>SERVICE TRAFFIC</th><th>ENFORCEMENT</th><th>ACTION</th></tr></thead><tbody>${rows}</tbody></table></section>`;
  }).join('');
...
async function disableThrottleTarget(target){if(!target)return;try{await api('/api/topology/throttle',{method:'DELETE',body:JSON.stringify({target})});hideModal('throttle-modal');await loadTopology();toast(`Throttle disabled for ${target}.`,'success');}catch(err){toast(`Failed to disable throttle: ${err.message}`);}}
function openThrottle(target){const enforcement=state.topology?.enforcement;if(!enforcement?.available){toast('XDP enforcement is disabled. Set BAZALT_THROTTLE_ENABLED=true and restart Bazalt.');return;}const rule=exactTopologyRule(target);$('#throttle-target').value=target;$('#throttle-target-label').textContent=target;const percent=Number(rule?.drop_percent||25);$('#throttle-percent').value=percent;$('#throttle-percent-range').value=percent;$('#throttle-ttl').value='300';$('#throttle-disable').classList.toggle('hidden',!rule);showModal('throttle-modal');}
...
$('#throttle-form').addEventListener('submit',async e=>{e.preventDefault();const target=$('#throttle-target').value;const drop_percent=Math.max(1,Math.min(100,Number($('#throttle-percent').value)||25));const ttl_seconds=Math.max(1,Math.min(3600,Number($('#throttle-ttl').value)||300));try{await api('/api/topology/throttle',{method:'PUT',body:JSON.stringify({target,drop_percent,ttl_seconds})});hideModal('throttle-modal');await loadTopology();toast(`Throttle ${drop_percent}% applied to ${target}.`,'success');}catch(err){toast(`Throttle failed: ${err.message}`);}});
$('#throttle-disable').addEventListener('click',()=>disableThrottleTarget($('#throttle-target').value));
